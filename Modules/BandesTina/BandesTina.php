...
class BandesTina extends Module{
	...
	/**
	 * Nom de l'administrateur local utilisé pour se connecter à distance ET pour lancer les commandes sur Tina.
	 * Tina ignore totalement l'existence du domaine, d'où la nécessité d'utiliser un compte local.
	 *
	 * @var string
	 */
	protected $tinaServerAdminLogin = 'administrateur';

	/**
	 * Mot de passe du compte d'administrateur local utilisé
	 *
	 * @var string
	 */
	protected $tinaServerAdminPwd = 'gru23mp18';
	...
	protected function refreshCartsList($force = false){
		...
		if ($force and ACL::canModify('module', $this->id)){
			...
			passthru('cat </dev/null | winexe --interactive=0 -U '.$this->tinaServerAdminLogin.'%'.$this->tinaServerAdminPwd.' //'.$server.' "cmd /C '.$filePath.'/'.$this->settings['scriptName']->getValue().'"', $retCode);
			if ($retCode != 0){
