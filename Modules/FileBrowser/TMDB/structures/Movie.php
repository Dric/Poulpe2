...
class Movie extends Asset {
	...
	public function alternative_titles($country = '') {
		...
		$info = $db->info(self::$type, $this->id, 'alternative_titles', array(
			'country' => $country
		));
		return $info;
	}
	...
	public function images($language = null, $size = false) {
		...
		$db = \TMDB\Client::getInstance();
		$info = $db->info(self::$type, $this->id, 'images', array(
			'language' => $language
		));
	...
	public function trailers($language = null) {
		$db = \TMDB\Client::getInstance();
		$info = $db->info(self::$type, $this->id, 'trailers', array(
			'language' => $language
		));
		return $info;
	}
	...
	public function similar_movies($language = null, $page = 1) {
		$db = \TMDB\Client::getInstance();
		$movies = array();
		$info = $db->info(self::$type, $this->id, 'similar_movies', array(
			'language' => $language,
			'page' => $page
		));
