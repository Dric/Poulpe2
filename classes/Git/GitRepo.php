 ...
 * @repo       https://github.com/together-networks/Git.php
 */
...
class GitRepo {
	...
	/**
	 * Retourne l'url de l'origine du dépôt
	 *
	 * @return string
	 */
	...
	/**
	 * Retourne un objet contenant le hash complet, le hash court, la date du commit et l'url vers l'interface web d'origine
	 * @return \stdClass
	 */
	public function getLastCommit(){
		$originURL = preg_replace('/.git$/', '', $this->getOrigin());
		$lastCommit = new \stdClass;
		list($lastCommit->fullHash, $lastCommit->hash, $lastCommit->date) = explode(' ', $this->run('log -1 --format="%H %h %ct"'));
		$lastCommit->date = (int)$lastCommit->date;
		$lastCommit->url = $originURL.'/commit/'.$lastCommit->fullHash;
