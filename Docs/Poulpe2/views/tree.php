    ...
    foreach ($array as $key => $item) {
        if (is_array($item)) {
            ...
            $selected = (isset($parts[$step]) && $item == $parts[$step]);
	          $itemTitle = pathinfo($item, PATHINFO_FILENAME);
	          if (strpos($itemTitle, '-') !== false){
		          list($num, $title) = explode('-', $itemTitle, 2);
	          }

            $t .= '<li class="file'. ($selected ? ' active' : '') .'"><a href="'. $parent .'/'. $item . '">'.(isset($num) ? $title : $itemTitle).'</a></li>';
        }
    }
...
<script>
    ...
    $(document).ready(function() {
        var iconFolderOpenClass  = 'glyphicon-folder-open',
            iconFolderCloseClass = 'glyphicon-folder-close',
